...
| C | `faaee63` | refactor: extract shared common schemas (4 types, ~78 lines) |
| D | (pending) | refactor: extract review/approval schemas (6 classes, ~85 lines) |
...
## Slice D: Review/Approval Schema Extraction

从 `schemas/__init__.py` 提取 6 个 Review/Approval schema 到 `schemas/review.py`。

### 变动
- 新增 `schemas/review.py` (85 lines)：DraftSummary、DraftsResponse、DraftDetailResponse、ApproveRequest、RejectRequest、ApprovalResponse
- `schemas/__init__.py` 从 1154 → 1091 lines (-63 lines)

### 原因
Slice C 将 SourcePathViewModel/StatusItem/NextAction 提取至 common.py 后，Review/Approval schema 的循环 import 障碍已解除。DraftSummary 是主路径核心 schema（贯穿 Drafts → Review → Approval → Library），其提取标志着 Review/Approval domain 边界正式确立。
...
| After Slice D | 1091 | 372 | 1463 |

注：总行数略增是因为每个子模块有独立的 docstring + import header，但 **主文件从 1375 → 1091 行（-21%）**。`__init__.py` 现在只包含 domain schema 定义 + re-exports。
