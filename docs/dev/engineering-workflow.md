...
Autopilot 不得在成功完成一轮 loop 后停止，如果下一步已经明确。

如果最终报告能写出"下一步是 X"，就必须继续执行 X。只有触发 hard-stop 边界才能停止。

**commit/push 不是停止点，是检查点。** commit/push 后：
1. 重新建立工程事实
2. 按需重新读取活跃 roadmap/spec/notes
3. 判断是否存在 hard-stop
4. 如无 hard-stop，继续下一轮

如果 context 不足，不是停在口头报告，而是写 handoff → commit/push → 明确要求新会话继续 `/mf-autopilot`。

停止原因必须是以下 hard-stop code 之一：`HARD_STOP_SECRET`、`HARD_STOP_REAL_LLM`、`HARD_STOP_PRIVATE_DATA`、`HARD_STOP_OBSIDIAN_WRITE`、`HARD_STOP_MAIL_STORAGE`、`HARD_STOP_RAG_EMBEDDING`、`HARD_STOP_LARGE_DEPENDENCY`、`HARD_STOP_APPROVAL_SEMANTICS`、`HARD_STOP_PRODUCT_DECISION`、`HARD_STOP_CONTEXT_LOW_HANDOFF_WRITTEN`、`HARD_STOP_P0_P1_RETRY_EXCEEDED`、`HARD_STOP_GIT_UNSAFE_STATE`。

### Context policy

- context ≥ 15%: 正常执行
- context < 15%: 不开始大型新实现单元
- context < 10%: 只完成当前单元、跑 gate、写 handoff、commit/push
- context < 5%: 立即写 handoff、跑最小 gate、commit/push、停止

### Roadmap-authorized execution mode

Roadmap / SPEC / Plan / implementation notes 是 Autopilot 的执行授权来源。只要工作属于 MindForge Roadmap / SPEC / Plan / implementation notes 中已经确立的产品方向，Autopilot 就可以自动推进，**不因 backend / API / schema / service / strategy / tests / Web / docs 改动而停止**。

普通 backend/API/service/strategy 改动不是停止条件，是正常可执行工作。

Backend/API work 自动允许，当且仅当：
1. 属于活跃 Roadmap / SPEC / Plan 范围
2. 保持 MindForge 产品大方向（个人 AI 学习记忆库 / 本地知识工作台）
3. 不触碰全局硬红线
...
Autopilot 适用于以下全部范围（自动继续，无需用户确认）：

| 层级 | 范围 |
|------|------|
| Backend | service / strategy / policy / presenter 层实现 |
| API | Web API schema 扩展、endpoint 新增/修改（需 spec 授权） |
| 数据 | schema 扩展、字段新增、类型定义 |
| 知识质量 | Card Quality、Wiki Quality（非 embedding/RAG 方案） |
| 关系 | Related Cards API（轻量/确定性的）、Source Location/Provenance |
| 健康 | Knowledge Health checks、维护报告 |
| 图谱 | Local Graph Preview（确定性图，非 vector/embedding） |
| 前端 | UX/UI/i18n/copy/Wiki/Library/Card UI |
| 测试 | Python tests、contract tests、fake dogfood、browser smoke |
| 文档 | docs/specs/plans/implementation notes/roadmap |

### 全局硬红线（始终生效）

只有以下情况 Autopilot 才自动停止并询问用户：

1. 需要真实 API key / secrets
2. 需要读取 `.env` / secrets
3. 需要调用真实 LLM / Cubox / Upstage（除非用户明确进入真实 dogfood）
4. 需要处理真实私人资料
5. 需要写真实 Obsidian vault
6. 需要 mail storage / email / mail 实现
7. 需要 RAG / embedding / vector DB
8. 需要新增大型框架 / 重依赖（除非先写 spec 明确说明）
...
- Roadmap 定义的 v0.3 工作
- 活跃 spec 要求的前后端集成
- 本地确定性图谱/关系计算（不使用 embedding/RAG/vector DB）

### 产品方向优先

全局硬红线优先于 roadmap。如果 roadmap 中某项工作与硬红线冲突，硬红线胜出。
