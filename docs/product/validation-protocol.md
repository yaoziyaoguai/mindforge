...
版本: v1.1
日期: 2026-05-28
状态: 就绪（等待真实用户 — HARD_STOP_PRODUCT_DECISION）
...
**核心假设：** 非技术用户能在首次使用 MindForge 时，在 15 分钟内独立完成完整的主路径循环（了解当前知识 → 导入一段内容 → 审阅 AI 生成的草稿 → 显式审批 → 浏览知识库 → 检索 → 生成总结 → 导出）。
...
| # | 判定条件 | 判定 |
|---|----------|------|
| K1 | ≥3/5 用户无法在 ≤15 min 内完成 tasks 2-4 (导入→审批→浏览) | 终止或重大调整 |
| K2 | ≥3/5 用户不理解或认为 explicit approval 没有价值 | 终止 |
| K3 | ≥3/5 用户说 "我不会用这个工具" | 终止 |
| K4 | 平均任务完成时间 > 25 min | 终止或重大调整 |

**有条件通过：** 2 人完成但 1 人失败 → 针对性修复 → re-validate
**通过：** ≥3/5 在 ≤15 min 完成 + NPS ≥ +30
...
| 结果 | 行动 |
|------|------|
| 通过 | P1 修复 validation 发现的问题 → 启动真实 LLM 集成 (Direction D) |
| 条件通过 | 针对性修复 → 第二轮 re-validation (新用户) |
| 终止 | 写 honest post-mortem → 评估 pivot 方案 → 决定 archive 或方向调整 |
...
在用户验证之前验证 demo workspace 和所有主路径页面在 auto-fallback fake provider 下正常工作。
