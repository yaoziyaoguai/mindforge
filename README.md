...
Current execution boundary: `process --strategy <id>` remains an advanced opt-in
debugging seam. The normal `import` / `watch add` card-writing path still uses
the default five-stage card renderer. MindForge does not yet expose `active_strategy`
in user config because non-five-stage envelopes need a
dedicated renderer before they can become first-class disk cards. This avoids a
half-working path where a strategy is selectable but cannot be safely written as
an AI Card.
