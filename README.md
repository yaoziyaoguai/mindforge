...
| 边界 | 说明 |
|------|------|
| local-first | 单用户本地 workspace；不上传 telemetry |
...
| Wiki 只用已审批卡片 | Wiki rebuild 不绕过 approval 读取 raw source |
...
| Graph 不是 GraphRAG | Local Graph Preview 是确定性局部关系图，不是 Vector DB、Graph DB 或 GraphRAG |
...
明确不做：not RAG / not embedding / no vector DB / no GraphRAG。当前 Recall 是 BM25 词法检索，Local Graph Preview 是确定性局部导航。
