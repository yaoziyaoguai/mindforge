...
4. 审批后的 `human_approved` 卡片进入 Library，可被 Recall 检索，并按 Topic 生成运行时知识视图。
...
- **Topic View**：按 Topic 分组浏览已审批卡片，通过 API (`/api/topics`) 获取安全运行时视图。
...
- **Local Graph Preview**：基于 source、tag、wiki section 等确定性关系展示局部关系图（card/source/tag/wiki_section 4 种节点类型）；不使用向量数据库，不是 GraphRAG。Graph/Sensemaking 是 lab/internal 功能，不在主导航中暴露，主入口在 Library 页面的 Graph Explorer。

## 当前范围与已知限制

- 已支持：Web Setup 配置真实模型、`import` / `watch add` 处理本地 source、生成 `ai_draft`、显式 approve、Library / BM25 Recall / Topic View、Knowledge Health、Local Graph Preview（4 NodeTypes）。
...
| 功能 | 状态 | 说明 |
|------|------|------|
| Graph Page（独立全页） | internal | 保留 `/graph` 路由但不在主导航；Library 页面的 GraphExplorer 是主入口 |
| Sensemaking Workspace | lab | 实验性分析（bridge detection/card evolution 等基于简单 heuristics），已从主导航隐藏 |
| Entity Resolution | lab | ConceptCandidate 确定性检测，不支持自动升级为 Entity |
...
| Graph 不是 GraphRAG | Local Graph Preview 是基于 same_source / same_tag / same_wiki_section 等 deterministic relations 的局部预览（当前正式支持 card/source/tag/wiki_section 4 种 NodeType）；Graph 独立页面和 Sensemaking 是 lab/internal，不在主导航，不是 Vector DB、Graph DB、Embedding 或 GraphRAG |
