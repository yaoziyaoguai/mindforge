...
- Historical safety boundary phrases for old fixture evidence: every dogfooding session must be a dry-run by default; does not call the real Cubox API; does not call a real LLM; does not print `.env` secret values; does not automatically modify a real private vault; does not auto-approve.
- Future gates remain closed unless explicitly shipped: G1 Real Cubox ingestion / Real Cubox ingestion / Real Cubox API / Real Cubox API calls enabled by default: No; G2 Real Obsidian formal-note write; G3 Approval UX; G4 Custom executable strategy runtime; G5 RAG / embedding / semantic merge; G6 Public release / git tag.
