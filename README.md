...
- `strategy.active`: 默认知识抽取策略；缺省为 `five_stage`。它只决定
  SourceDocument 如何变成 AI Card envelope，不决定 provider/model。
...
| `mindforge strategies list` | 只读查看 built-in strategies 及 implemented/preview/planned 状态 |
| `mindforge strategies show five_stage` | 只读查看某个 strategy 的版本、安全边界和 prompt version 线索 |
...
Built-in strategies include `five_stage`, `default_knowledge_card`,
`concept_extraction`, and planned metadata such as `action_item`.
...
the default `five_stage` strategy persist strategy/prompt/source/provider provenance
...
```bash
mindforge import ./note.md --strategy default_knowledge_card --provider fake
mindforge watch add ./notes --strategy five_stage
mindforge process --strategy concept_extraction
```
...
built-in default `five_stage`. Provider selection remains separate:
`--provider` / `llm.active` choose the LLM provider/model; `--strategy` /
`strategy.active` choose the extraction strategy. Planned/custom declarative
strategies are visible through `strategies list/show`, but are not executed.
`default_knowledge_card` and `concept_extraction` are deterministic strategies;
their generated cards use empty `prompt_versions` / `stage_models` instead of
pretending to have run the five-stage prompt set.
