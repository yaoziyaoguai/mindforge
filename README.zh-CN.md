...
| 边界 | 说明 |
|------|------|
| 默认不联网 | fresh workspace 默认不调用真实 LLM/API，也不上传 telemetry；外部模型调用必须由用户显式配置 provider/API key 并主动触发 import/watch processing |
...
| Topic View 只用已审批卡片 | Topic View 不绕过 approval，不从未审批内容构建，也不读取 raw source |
| 真实模型必须显式触发 | 配置模型 + API key + 用户触发 import/watch processing |
...
Topic View 是 **运行时视图**：从 `human_approved` cards 按 Topic 分组，通过 `/api/topics` API 获取。CLI：
...
Topic View 只从 approved cards 生成，不绕过 approval 读取 raw source。Approved cards 是 source of truth，Topic View 是派生的运行时视图。LLM-based Wiki synthesis (`llm_rebuild_wiki`) 已在 v0.5 废弃。
