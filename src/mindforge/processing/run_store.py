"""ProcessingRun 持久化与后台执行 — core 层。

中文学习型说明：ProcessingRun 是用户可见的异步任务记录，不替代 ``RunLogger``。
RunLogger 记录 pipeline 内部事件；ProcessingRun 记录用户点击 Process now 后能看到的
任务状态、摘要、跳过原因和错误。这里不存 raw source、不存 prompt、不存 API key，
只保存用户可见的安全元数据。

此模块是 core 层（src/mindforge/）的一等公民，不依赖 web 层。
Web 层的 processing_run_service 是对此模块的 thin re-export shim。
"""
