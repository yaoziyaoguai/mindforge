...
def load_prompt(prompts_dir: Any, stage: str, version: str) -> str:
    """读取 ``<prompts_dir>/<stage>/<version>.md`` 全文。

    中文学习型说明：v0.5.2 允许 ``prompts_dir`` 是普通 ``Path``，也可以是
    ``importlib.resources.files(...)`` 返回的 Traversable。这样 packaged
    install 不再依赖 repo root，同时用户显式传入 ``--prompts-dir`` 的旧路径
    仍然照常工作。
    """
    if isinstance(prompts_dir, Path):
        path = prompts_dir / stage / f"{version}.md"
    else:
        path = prompts_dir.joinpath(stage, f"{version}.md")
