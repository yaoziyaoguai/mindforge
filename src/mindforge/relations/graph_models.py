...
- NodeType 从 5 个扩展到 9 个：新增 COMMUNITY, TOPIC, ENTITY，CONCEPT 重命名为 CONCEPT_CANDIDATE
- EdgeType 从 9 个重新组织为 15 个：新增 HAS_TAG, IN_SECTION, CONTAINS, INCLUDES,
...
class NodeType(str, enum.Enum):
    ...
    Fact graph nodes（有确定性 evidence 的已确认节点）:
      CARD, SOURCE, WIKI_SECTION, TAG, COMMUNITY, TOPIC, ENTITY

    Candidate graph nodes（待用户确认的候选节点）:
      CONCEPT_CANDIDATE
    """
...
class Graph:
    ...
    v3.7 升级：center_type 支持所有 9 种 NodeType，不再局限于 CARD。
    """
