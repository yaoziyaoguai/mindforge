...
把 [V0_14_FUTURE_GATES.md](../docs/V0_14_FUTURE_GATES.md) 里 G1–G6
gate **之前不允许越界落地**的具体实现形态固化为仓库级源码扫描断言。
...
如果未来某个 gate 被 human authorizer 正式打开:
1. 在对应 gate doc 里写 RFC;
2. 在 ROADMAP_COMPLETION_LEDGER.md 里把对应 row 从 ``future-gated``
   迁出;
3. 同时在本测试中**显式 allowlist 该新模块**, 而不是删掉断言。
   "靠删测试通过 gate" 本身是 P0 违规。
"""
...
def test_completion_ledger_doc_exists_with_required_buckets():
    ...
    # 必须 cross-link future gates
    assert "V0_14_FUTURE_GATES.md" in text
